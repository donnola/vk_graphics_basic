...
void SimpleCompute::SetupSimplePipeline()
{
  ...
  m_A_shader = vk_utils::createBuffer(m_device, sizeof(float) * m_length, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
                                                                       VK_BUFFER_USAGE_TRANSFER_DST_BIT);
  m_B_shader = vk_utils::createBuffer(m_device, sizeof(float) * m_length, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
                                                                       VK_BUFFER_USAGE_TRANSFER_SRC_BIT);
  vk_utils::allocateAndBindWithPadding(m_device, m_physicalDevice, {m_A_shader, m_B_shader}, 0);
  ...
  m_pBindings->BindBuffer(0, m_A_shader);
  m_pBindings->BindBuffer(1, m_B_shader);
...
void SimpleCompute::CleanupPipeline()
{
  ...
  vkDestroyBuffer(m_device, m_A_shader, nullptr);
  vkDestroyBuffer(m_device, m_B_shader, nullptr);
...
void SimpleCompute::Execute()
{
  ...
  std::vector<float> values(m_length);
  m_pCopyHelper->ReadBuffer(m_B_shader, 0, values.data(), sizeof(float) * values.size());
  float sum1 = 0;
  for (auto v: values) {
    sum1 += v;
  }
